...
<script type=text/javascript src="${pageContext.request.contextPath}/resource/daumOpenEditor/js/jquery.form.min.js"></script>

   <link rel=stylesheet type=text/css href="${pageContext.request.contextPath}/resource/daumOpenEditor/css/popup.css" charset=utf-8 /> 
<script type=text/javascript charset=utf-8 src="${pageContext.request.contextPath}/resource/daumOpenEditor/js/popup.js"></script>
 <!-- jQuery 라이브러리 --> 
 <script type=text/javascript src="${pageContext.request.contextPath}/resource/daumOpenEditor/js/jquery-1.11.0.min.js"></script>
 
 <style> /* css */ .header { 
 background-image: none; background-color: #027dfc; } 
 /* 파일첨부(.file) */ .file { display: inline-block; margin-top: 8px; overflow: hidden; } 
 ...
 $(document).ready(function (){ // <input type=file> 태그 기능 구현 
	 $('.file input[type=file]').change(function (){ var inputObj = $(this).prev().prev(); // 두번째 앞 형제(text) 객체 
	...
	// 등록버튼 클릭 이벤트 
	$('.submit a').on('click', function () { var form = $('#daumOpenEditorForm'); // form id값 
	...
	form.ajaxSubmit({ type: 'POST', url: '${pageContext.request.contextPath}/daumOpenEditor/singleUploadImageAjax', dataType: 'JSON', // 리턴되는 데이타 타입 
		beforeSubmit: function() { if(validation(fileName)) { // 확장자 체크 (jpg, gif, png, bmp)
		return false; } }, success: function(fileInfo) { // fileInfo는 이미지 정보를 리턴하는 객체 
			if(fileInfo.result===-1) { // 서버단에서 체크 후 수행됨 
				alert('jpg, gif, png, bmp 확장자만 업로드 가능합니다.'); return false; } else if(fileInfo.result===-2) { // 서버단에서 체크 후 수행됨 
					alert('파일이 1MB를 초과하였습니다.'); return false; } else { done(fileInfo); // 첨부한 이미지를 에디터에 적용시키고 팝업창을 종료 
		} } }); });
   ...
   <!-- 다음오픈에디터 라이브러리 -->
   <link rel="stylesheet" href="/SecondPrj/src/main/webapp/resource/daumOpenEditor/css/popup.css" type="text/css"  charset="utf-8"/>
   <script src="/SecondPrj/src/main/webapp/resource/daumOpenEditor/js/popup.js" type="text/javascript" charset="utf-8"></script>
   ...
   if(!((fileNameExtension === 'jpg') || (fileNameExtension === 'gif') || (fileNameExtension === 'png') || (fileNameExtension === 'bmp'))) { alert('jpg, gif, png, bmp 확장자만 업로드 가능합니다.'); return true; } else { return false; } }
      
      
   </script>


<main onload="initUploader();">
   ...
   <div class="wrapper">
      ...
      <div class="body">
         ...
         <dt>nbsp;1MB이하 (JPG,GIF,PNG,BMP)</dt>
          ...
          <input id=uploadInputBox style="display: none" type=file name=Filedata><!-- 버튼대체용(안보임) --> </div> <!-- //파일첨부 --> </form> </dd> </dl>
      </div>
      <div class="footer">
        <!--  <p>
